...
public class Controleur
{
	...
	/* méthode qui renvoie le nom du joueur Courant */
	public String getNomJoueurCourant()
	{
		if ( this.metier == null || this.plateau == null )
			return "";

		return this.metier.getJoueurCourant().getNom();
	}


	/* méthode qui renvoie le nombre de joueurs dans la partie */
	public int getNbJoueurs()
	{
		if ( this.metier == null || this.plateau == null )
			return 0;

		return this.metier.getNbJoueurs();
	}

	/* méthode qui renvoie le joueurs courants */
	...
	public void setCheminPlateau ( String chemin )
	{
		...
		this.ihm.getPanelChargementDuo().chargerPlateau(); // <- charge prévisualisation pour les deux plateau du panelChargementduo 
	}
	...
	public boolean estDuo()
	{
		return this.metier != null && this.metier.getNbJoueurs() == 2;
	}
	...
	public void reinitialiserFanionDuo()
	{
		if (this.metier == null || this.metier.getMancheCourante() == null)
			return;

		this.metier.getMancheCourante().reinitialiserFanionCourant();

		// Remettre les deux joueurs en état de piocher
	...
	public void bloquerPiocheAutresJoueurs()
	{
		for (Joueur j : this.metier.getJoueurs())
			if (j != this.metier.getJoueurCourant())
				j.setAPioche();
	}

	public List<int[]> getVoisinDisponibleJoueur(int lig, int col, int indexJoueur)
	{
		List<int[]> lstVoisinDispo = new ArrayList<>();
		...
		Joueur joueur = this.metier.getJoueurs().get(indexJoueur);

		if (!joueur.estExtremiteJouable(balise))
			return lstVoisinDispo;

		for (Balise baliseVoisine : balise.getVoisins())
		{
			if (!joueur.aVisite(baliseVoisine)              &&
				fanion.correspond(baliseVoisine.getNumero()) &&
				!joueur.segmentSeCroise(balise, baliseVoisine))
			{
				lstVoisinDispo.add(new int[]{ baliseVoisine.getPosition().getLigne(), baliseVoisine.getPosition().getColonne() });
			}
		}
	...
	public void capturerBaliseJoueur(int ligDepart, int colDepart, int ligArrive, int colArrive, int indexJoueur)
	{
		if (this.metier == null)
			return;
		...
		Joueur joueur = this.metier.getJoueurs().get(indexJoueur);
		joueur.capturer(baliseDepart, baliseArrive);
		joueur.setAJoue();
	}

	public int getScoreJoueur(int indexJoueur)
	{
		if (this.metier == null)
			return 0;
		return this.metier.getJoueurs().get(indexJoueur).calculerScoreManche();
	}
	...
	/* méthode qui créer une partie duo */
	public void creerPartieDuo(String nomJ1, String nomJ2)
	{
		// this.plateau est déjà chargé depuis le panelChargementDuo
		List<BaliseDepart> lstBaliseDeparts = this.plateau.getBalisesDepart();

		if (lstBaliseDeparts == null || lstBaliseDeparts.isEmpty())
			return;
	...
	public int getScoreJoueurTotal(int indexJoueur)
	{
		if (this.metier == null)
			return 0;

		Joueur j = this.metier.getJoueurs().get(indexJoueur);
		return j.getScoreTotal() + j.calculerScoreManche();
	}
