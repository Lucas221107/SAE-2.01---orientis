...
public class PanelPlateau extends JPanel
{
	...
	private class GererSouris extends MouseAdapter
	{
		...
		{
			/* coordonnéé du click sur le panel */
			int x = e.getX();
			int y = e.getY();

			/* si le plateau ou la partie est null on ne touche à rien */
			if ( PanelPlateau.this.plateau == null || PanelPlateau.this.ctrl.getPartie() == null )
				return; 

			/* calcul du décallage pour centrer le plateau */
			...
			/* calcul pour savoir la igne et la colonne du click sur le plateau */
			int lig = ( y - decallageY ) / tailleCase;
			int col = ( x - decallageX ) / tailleCase;

			/* Si le click est hors du tableau on ne fait rien */
			if ( lig < 0 || lig > PanelPlateau.this.plateau.getNbLignes() || 
				 col < 0 || col > PanelPlateau.this.plateau.getNbColonnes() )
				 return;

			/* si le click n'est pas sur une case ou il y a une balise on ne fait rien */
			...
			if ( PanelPlateau.this.tabBaliseSelectionnee == null ) 
			{
				...
				/* si une carte n'a pas été pioché on ne fait rien */
				if ( PanelPlateau.this.ctrl.getNumeroFanionCourant() == -1 )
					return;

				PanelPlateau.this.tabBaliseSelectionnee = new int[] { lig, col }; // <- ajoute les coordonnées de la balise clické dans la tabBaliseSelectionnée
				PanelPlateau.this.lstBaliseDisponible   = PanelPlateau.this.ctrl.getVoisinDisponible(lig, col); // <- remplie la liste de voisin disponible
				PanelPlateau.this.repaint(); // <- redessine le plateau
				return;
			}

			/* pour toutes les balises disponible dans la liste de voisin disponible */
			for ( int[] baliseDispo : PanelPlateau.this.lstBaliseDisponible)
			{
				/* on vériffie que les coordonnées corespondent à la lig et la col */
				if ( baliseDispo[0] == lig && baliseDispo[1] == col )
				{
					...
					/* On capture la baliseDispo */
					PanelPlateau.this.ctrl.capturerBalise( ligBaliseSelectionnee, colBaliseSelectionnee, lig, col);

					/* On ajoute les coordonnées du segment de capture dans la liste ainsi que la couleur du traits */
					PanelPlateau.this.lstSegmentDessines.add ( new int[]  { ligBaliseSelectionnee, colBaliseSelectionnee, lig, col, PanelPlateau.this.ctrl.getNumeroManche()});

					/* on réinitialise le tableau de balise Selectionnée ainsi que la liste de voisin disponibke */
					PanelPlateau.this.tabBaliseSelectionnee = null;
					PanelPlateau.this.lstBaliseDisponible.clear();

					/* Si la manche est terminé */
					if ( PanelPlateau.this.ctrl.estMancheTermine())
					{
						...
						PanelPlateau.this.frame.getPanelSolo().ajouterScoreManche(numManche, score); // <- ajoute le score de la manche au panelSolo
						
						/* On redemarre une nouvelle manche */
			...
			/* on reinitialise le tableau de balise selectionnée ainsi que la liste */
			PanelPlateau.this.tabBaliseSelectionnee = null;
			PanelPlateau.this.lstBaliseDisponible.clear();
			PanelPlateau.this.repaint(); // <- on redessine le plateau
	...
	public void paintComponent ( Graphics g)
	{
		...
		if ( this.ctrl.getPartie() != null)
		{
			...
			int x = decallageX + this.ctrl.getColBaliseDepartJoueur() * this.tailleCasePlateau;
			int y = decallageY + this.ctrl.getLigBaliseDepartJoueur() * this.tailleCasePlateau;
